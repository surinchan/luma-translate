...
Windows 版本通过低级鼠标钩子识别拖动选区，并使用兼容不同桌面框架的系统复制机制读取文本。程序优先等待用户自己的 Ctrl+C，并在检测到真实键盘输入时取消自动复制，避免干扰正常快捷键。普通单击或没有产生新复制内容时不会显示翻译按钮；鼠标再次点击或超时后按钮会立即隐藏。macOS 和 Linux 目前同样使用系统复制机制。
